    ...
    def remap(self, size=-1):
        ...
        try:
            ...
            if size == 0:
                ...
                self.data = mmap.mmap(self.file, size, mmap.MAP_SHARED,
                                      mmap.PROT_READ | mmap.PROT_WRITE if self.writable else mmap.PROT_READ)
                self.size = size
        except Exception:
            self.unmap()
            raise
...
def _decode_datetime(value):
    return datetime.datetime(1900 + (value >> 53), (value >> 49) & 0b1111, (value >> 44) & 0b11111,
                             (value >> 39) & 0b11111, (value >> 33) & 0b111111, (value >> 27) & 0b111111,
                             ((value >> 17) & 0b1111111111) * 1000)


def _decode_time(value):
    return datetime.time(value >> 26, (value >> 20) & 0b111111, (value >> 14) & 0b111111,
                         ((value >> 4) & 0b1111111111) * 1000)
...
class ProcData(_SingletonType("_Singleton", (object,), {})):
    ...
    class Column(collections.Sequence):
        def __init__(self, file, writable):
            ...
            if self._type == ProcData.ColumnType.BYTES:
                ...
                self._var_type = True
                self._decode_var_value = lambda var_data, start, end: b"" if start == end else \
                struct.unpack_from(str(end - start) + "s", var_data, start)[0]
            elif self.type == ProcData.ColumnType.STRING:
                self._var_string = True
                self._var_type = True
                self._decode_var_value = lambda var_data, start, end: "" if start == end else _decode_string(
                    struct.unpack_from(str(end - start - 1) + "s", var_data, start)[0])
            else:
                self._var_string = None
                self._var_type = False

                self._decode_value = {
                    ProcData.ColumnType.CHAR1:     lambda data, index: _decode_char(
                        _char1_struct.unpack_from(data, index)[0]),
                    ProcData.ColumnType.CHAR2:     lambda data, index: _decode_char(
                        _char2_struct.unpack_from(data, index * 2)[0]),
                    ProcData.ColumnType.CHAR4:     lambda data, index: _decode_char(
                        _char4_struct.unpack_from(data, index * 4)[0]),
                    ProcData.ColumnType.CHAR8:     lambda data, index: _decode_char(
                        _char8_struct.unpack_from(data, index * 8)[0]),
                    ProcData.ColumnType.CHAR16:    lambda data, index: _decode_char(
                        _char16_struct.unpack_from(data, index * 16)[0]),
                    ProcData.ColumnType.CHAR32:    lambda data, index: _decode_char(
                        _char32_struct.unpack_from(data, index * 32)[0]),
                    ProcData.ColumnType.CHAR64:    lambda data, index: _decode_char(
                        _char64_struct.unpack_from(data, index * 64)[0]),
                    ProcData.ColumnType.CHAR128:   lambda data, index: _decode_char(
                        _char128_struct.unpack_from(data, index * 128)[0]),
                    ProcData.ColumnType.CHAR256:   lambda data, index: _decode_char(
                        _char256_struct.unpack_from(data, index * 256)[0]),
                    ProcData.ColumnType.DATE:      lambda data, index: _decode_date(
                        _int32_struct.unpack_from(data, index * 4)[0]),
                    ProcData.ColumnType.DATETIME:  lambda data, index: _decode_datetime(
                        _int64_struct.unpack_from(data, index * 8)[0]),
                    ProcData.ColumnType.DECIMAL:   lambda data, index: decimal.Decimal(
                        _int64_struct.unpack_from(data, index * 8)[0]).scaleb(-4),
                    ...
                    ProcData.ColumnType.TIME:      lambda data, index: _decode_time(
                        _uint32_struct.unpack_from(data, index * 4)[0]),
                    ProcData.ColumnType.TIMESTAMP: lambda data, index: _int64_struct.unpack_from(data, index * 8)[0]
                }[self._type]

                self._decode_multiple = {
                    ProcData.ColumnType.CHAR1:     lambda data, index, count: [_decode_char(value) for value in
                                                                               struct.unpack_from(str(count) + "c",
                                                                                                  data, index)],
                    ProcData.ColumnType.CHAR2:     lambda data, index, count: [_decode_char(value) for value in
                                                                               struct.unpack_from("2s" * count, data,
                                                                                                  index * 2)],
                    ProcData.ColumnType.CHAR4:     lambda data, index, count: [_decode_char(value) for value in
                                                                               struct.unpack_from("4s" * count, data,
                                                                                                  index * 4)],
                    ProcData.ColumnType.CHAR8:     lambda data, index, count: [_decode_char(value) for value in
                                                                               struct.unpack_from("8s" * count, data,
                                                                                                  index * 8)],
                    ProcData.ColumnType.CHAR16:    lambda data, index, count: [_decode_char(value) for value in
                                                                               struct.unpack_from("16s" * count, data,
                                                                                                  index * 16)],
                    ProcData.ColumnType.CHAR32:    lambda data, index, count: [_decode_char(value) for value in
                                                                               struct.unpack_from("32s" * count, data,
                                                                                                  index * 32)],
                    ProcData.ColumnType.CHAR64:    lambda data, index, count: [_decode_char(value) for value in
                                                                               struct.unpack_from("64s" * count, data,
                                                                                                  index * 64)],
                    ProcData.ColumnType.CHAR128:   lambda data, index, count: [_decode_char(value) for value in
                                                                               struct.unpack_from("128s" * count, data,
                                                                                                  index * 128)],
                    ProcData.ColumnType.CHAR256:   lambda data, index, count: [_decode_char(value) for value in
                                                                               struct.unpack_from("256s" * count, data,
                                                                                                  index * 256)],
                    ProcData.ColumnType.DATE:      lambda data, index, count: [_decode_date(value) for value in
                                                                               struct.unpack_from(
                                                                                   "=" + str(count) + "i", data,
                                                                                   index * 4)],
                    ProcData.ColumnType.DATETIME:  lambda data, index, count: [_decode_datetime(value) for value in
                                                                               struct.unpack_from(
                                                                                   "=" + str(count) + "q", data,
                                                                                   index * 8)],
                    ProcData.ColumnType.DECIMAL:   lambda data, index, count: [decimal.Decimal(value).scaleb(-4) for
                                                                               value in struct.unpack_from(
                            "=" + str(count) + "q", data, index * 8)],
                    ProcData.ColumnType.DOUBLE:    lambda data, index, count: list(
                        struct.unpack_from("=" + str(count) + "d", data, index * 8)),
                    ProcData.ColumnType.FLOAT:     lambda data, index, count: list(
                        struct.unpack_from("=" + str(count) + "f", data, index * 4)),
                    ProcData.ColumnType.INT:       lambda data, index, count: list(
                        struct.unpack_from("=" + str(count) + "i", data, index * 4)),
                    ProcData.ColumnType.INT8:      lambda data, index, count: list(
                        struct.unpack_from("=" + str(count) + "b", data, index)),
                    ProcData.ColumnType.INT16:     lambda data, index, count: list(
                        struct.unpack_from("=" + str(count) + "h", data, index * 2)),
                    ProcData.ColumnType.IPV4:      lambda data, index, count: list(
                        struct.unpack_from("=" + str(count) + "i", data, index * 4)),
                    ProcData.ColumnType.LONG:      lambda data, index, count: list(
                        struct.unpack_from("=" + str(count) + "q", data, index * 8)),
                    ProcData.ColumnType.TIME:      lambda data, index, count: [_decode_time(value) for value in
                                                                               struct.unpack_from(
                                                                                   "=" + str(count) + "I", data,
                                                                                   index * 4)],
                    ProcData.ColumnType.TIMESTAMP: lambda data, index, count: list(
                        struct.unpack_from("=" + str(count) + "q", data, index * 8))
                }[self._type]
            ...
            if isinstance(index, _integer_types):
                ...
                if index.step is None or index.step == 1:
                    ...
                    if not self._var_type:
                        ...
                        if (stop < size):
                            positions = struct.unpack_from("=" + str(stop - start + 1) + "Q", self._data.data,
                                                           start * 8)
                        else:
                            positions = list(
                                struct.unpack_from("=" + str(stop - start) + "Q", self._data.data, start * 8))
                            positions.append(var_data.size)

                        result = [decode_var_value(var_data.data, positions[i], positions[i + 1]) for i in
                                  xrange(0, len(positions) - 1)]
    ...
    class OutputColumn(Column):
        def __init__(self, file):
            ...
            if not self._var_type:
                self._encode_value = {
                    ProcData.ColumnType.CHAR1:     lambda data, index, value: _char1_struct.pack_into(data, index,
                                                                                                      _encode_char(
                                                                                                          value, 1)),
                    ProcData.ColumnType.CHAR2:     lambda data, index, value: _char2_struct.pack_into(data, index * 2,
                                                                                                      _encode_char(
                                                                                                          value, 2)),
                    ProcData.ColumnType.CHAR4:     lambda data, index, value: _char4_struct.pack_into(data, index * 4,
                                                                                                      _encode_char(
                                                                                                          value, 4)),
                    ProcData.ColumnType.CHAR8:     lambda data, index, value: _char8_struct.pack_into(data, index * 8,
                                                                                                      _encode_char(
                                                                                                          value, 8)),
                    ProcData.ColumnType.CHAR16:    lambda data, index, value: _char16_struct.pack_into(data, index * 16,
                                                                                                       _encode_char(
                                                                                                           value, 16)),
                    ProcData.ColumnType.CHAR32:    lambda data, index, value: _char32_struct.pack_into(data, index * 32,
                                                                                                       _encode_char(
                                                                                                           value, 32)),
                    ProcData.ColumnType.CHAR64:    lambda data, index, value: _char64_struct.pack_into(data, index * 64,
                                                                                                       _encode_char(
                                                                                                           value, 64)),
                    ProcData.ColumnType.CHAR128:   lambda data, index, value: _char128_struct.pack_into(data,
                                                                                                        index * 128,
                                                                                                        _encode_char(
                                                                                                            value,
                                                                                                            128)),
                    ProcData.ColumnType.CHAR256:   lambda data, index, value: _char256_struct.pack_into(data,
                                                                                                        index * 256,
                                                                                                        _encode_char(
                                                                                                            value,
                                                                                                            256)),
                    ProcData.ColumnType.DATE:      lambda data, index, value: _int32_struct.pack_into(data, index * 4,
                                                                                                      _encode_date(
                                                                                                          value)),
                    ProcData.ColumnType.DATETIME:  lambda data, index, value: _int64_struct.pack_into(data, index * 8,
                                                                                                      _encode_datetime(
                                                                                                          value)),
                    ProcData.ColumnType.DECIMAL:   lambda data, index, value: _int64_struct.pack_into(data, index * 8,
                                                                                                      long(
                                                                                                          value * 10000)),
                    ProcData.ColumnType.DOUBLE:    lambda data, index, value: _double_struct.pack_into(data, index * 8,
                                                                                                       value),
                    ProcData.ColumnType.FLOAT:     lambda data, index, value: _float_struct.pack_into(data, index * 4,
                                                                                                      value),
                    ProcData.ColumnType.INT:       lambda data, index, value: _int32_struct.pack_into(data, index * 4,
                                                                                                      int(value)),
                    ProcData.ColumnType.INT8:      lambda data, index, value: _int8_struct.pack_into(data, index,
                                                                                                     int(value)),
                    ProcData.ColumnType.INT16:     lambda data, index, value: _int16_struct.pack_into(data, index * 2,
                                                                                                      int(value)),
                    ProcData.ColumnType.IPV4:      lambda data, index, value: _int32_struct.pack_into(data, index * 4,
                                                                                                      int(value)),
                    ProcData.ColumnType.LONG:      lambda data, index, value: _int64_struct.pack_into(data, index * 8,
                                                                                                      long(value)),
                    ProcData.ColumnType.TIME:      lambda data, index, value: _uint32_struct.pack_into(data, index * 4,
                                                                                                       _encode_time(
                                                                                                           value)),
                    ProcData.ColumnType.TIMESTAMP: lambda data, index, value: _int64_struct.pack_into(data, index * 8,
                                                                                                      long(value))
                }[self._type]
